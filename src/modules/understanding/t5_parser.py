...
from src.shared.constants import DEFAULT_UNDERSTANDING_CHECKPOINT, SCENE_COLORS
...
_DEFAULT_MODEL = Path(DEFAULT_UNDERSTANDING_CHECKPOINT)
...
class T5SceneParser:
    ...
    def __init__(
        ...
        # ARCH CONSTRAINT: flan-T5-small max input is 512 tokens.
        # 256 tokens ≈ ~180 English words — sufficient for scene prompts.
        # Increasing beyond 512 would need a different T5 variant (base/large).
        max_input_len: int = 256,
        # ARCH CONSTRAINT: T5 decoder max output is 512 tokens.
        # Scene JSON rarely exceeds ~200 tokens, but 512 allows complex multi-entity scenes.
        max_output_len: int = 512,
        # Beam search width: 4 beams is standard for T5 generation.
        # Higher values (8-16) improve JSON correctness but 4× slower per beam.
        num_beams: int = 4,
        fallback: bool = True,
    ) -> None:
    ...
    def parse(self, prompt: str) -> ParsedScene:
        ...
        if not prompt:
            log.warning("T5SceneParser: empty prompt — returning minimal scene")
            return ParsedScene(prompt=prompt)
        if not self.ensure_loaded():
            return self.do_fallback(prompt)

        try:
            raw_json = self.run_inference(prompt)
            if (data := self.safe_json_loads(raw_json)) is None:
                log.warning("T5 output is not valid JSON — falling back")
                return self.do_fallback(prompt)
            scene = self.dict_to_parsed_scene(prompt, data)

            # Augment: if T5 found entities but 0 actions, use rules-parser
            if not scene.actions and self._fallback_parser is not None:
                if (fb := self._fallback_parser.parse(prompt)).actions:
    ...
    def run_inference(self, prompt: str) -> str:
        assert self._tokenizer is not None, "call ensure_loaded() first"
        assert self._model is not None, "call ensure_loaded() first"

        inputs = self._tokenizer(  # type: ignore[operator]  # pyright: ignore[reportCallIssue]
        ...
        with torch.no_grad():
            ids = self._model.generate(  # pyright: ignore[reportOptionalMemberAccess]
                **inputs,
        ...
        # survive long enough for postprocess to reverse them into { / }
        raw = self._tokenizer.decode(ids[0], skip_special_tokens=False)  # type: ignore[union-attr]  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def dict_to_parsed_scene(self, prompt: str, data: dict) -> ParsedScene:
        """Convert the v5 slim schema dict into a ``ParsedScene`` dataclass."""
        entities = [self._build_entity(e) for e in data.get("entities", [])]
        actions, spatial_relations = self._extract_relations(data)
        return ParsedScene(
            prompt=prompt,
            entities=entities,
            actions=actions,
    ...
    def _build_entity(self, e: dict) -> ParsedEntity:
        """Convert a raw entity dict to a ParsedEntity."""
        name = e.get("name", "unknown")
        etype = e.get("type", "object").lower()

        if etype not in OBJECTS:
            if obj_def := get_object_by_keyword(etype):
                etype = obj_def.name
            if etype not in OBJECTS:
                for word in name.lower().split():
                    if obj_def := get_object_by_keyword(word):
                        etype = obj_def.name
                        break
        ...
        if (vocab_obj := OBJECTS.get(etype)) and vocab_obj.category.name == "HUMANOID":
            is_actor = True

        return ParsedEntity(
            name=name, object_type=etype,
            is_actor=is_actor, color=self.extract_color(name),
        )

    @staticmethod
    def _extract_relations(data: dict):
        """Split relations into actions and spatial relations."""
        ...
        for r in data.get("relations", []):
            ...
            if pred in _ACTION_PREDICATES:
                canon: str = _PRED_TO_ACTION.get(pred, pred)  # pyright: ignore[reportAssignmentType]
                actions.append(ParsedAction(
                    action_type=canon, actor=subj, target=obj,
                ))
            else:
                spatial_relations.append({
                    "subject": subj, "predicate": pred, "object": obj,
                })
        return actions, spatial_relations
    ...
    @staticmethod
    def extract_color(name: str) -> tuple[float, float, float, float] | None:
        """Find a known colour word inside the entity name."""
        lower = name.lower()
        for cname, rgba in SCENE_COLORS.items():
            if cname in lower:
                return rgba
        return None

    def do_fallback(self, prompt: str) -> ParsedScene:
