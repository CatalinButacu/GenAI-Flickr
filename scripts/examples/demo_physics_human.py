...
Runs the FULL pipeline for a text prompt:
  ...
  M1 → M2 → M4 → M5 → (M8) → M7
  NLP  scene  AMASS   PyBullet  ControlNet  cinematic
...
Physics constraints (in both paths):
  ...
  - RSI root tracking of SMPL-X pelvis trajectory
...
def main() -> None:
    ...
    config = PipelineConfig(
        ...
        use_motion_generation=True,   # M4: AMASS + SSM
        use_diffusion=use_ai,    # M8: AnimateDiff or ControlNet
