...
data/ (încărcare date KIT-ML) & 175 \\
...
\begin{itemize}
    \item \textbf{4.888 secvențe} de motion capture la 20 fps \cite{plappert2016kit}
    \item \textbf{251 dimensiuni} per cadru (unghiuri articulare, poziție root, velocitate) --- formatul HumanML3D \cite{guo2022humanml3d}
...
% ─────────────────────────────────────────────────────────────────────────
\section{Modelul corporal SMPL}

Pentru randarea realistă a mișcării umane, am integrat modelul parametric \textbf{SMPL} (Skinned Multi-Person Linear Model) \cite{loper2015smpl}.  Implementarea noastră este complet în NumPy (fără dependență de chumpy sau GPU), cu forward kinematics propriu:
...
Integrarea SMPL a necesitat câteva corecturi critice:

\begin{enumerate}
    \item \textbf{Convenția de axe} --- KIT-ML folosește Y-up, SMPL presupune Z-up; am aplicat o transformare de bază
...
\subsection{Retargetarea mișcării}

Modulul \texttt{motion\_retarget.py} (252 linii) convertește vectorii de mișcare KIT-ML (251-dim) în pose SMPL:
\begin{itemize}
    \item Mapare semantică de la articulațiile KIT-ML la cele SMPL (ex: \texttt{RightUpLeg} $\rightarrow$ SMPL joint 2)
    \item Conversie unghiuri Euler ($\rightarrow$ axis-angle) cu tratarea corectă a gimbal lock
