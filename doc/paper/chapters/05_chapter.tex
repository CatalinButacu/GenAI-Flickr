...
Verifică importuri, existența datasetului KIT-ML, fișierele \texttt{Mean.npy} și \texttt{Std.npy} cu forma $(251,)$, inițializarea generatorului cu \texttt{use\_retrieval=True} și \texttt{use\_ssm=True}, existența checkpoint-ului SSM.

\subsection{Teste de recuperare semantică (16--22)}

Testează recuperarea din KIT-ML pentru 7 acțiuni: \textit{walk forward, run fast, kick a ball, jump up, turn around, wave hand, stand still}.  Fiecare test verifică că sursa clipului este \texttt{``retrieved''} sau \texttt{``semantic\_retrieved''}.
...
\begin{itemize}
    \item \textbf{Forma output-ului}: 100 cadre $\times$ 251 dimensiuni
    \item \textbf{FPS corect}: 20 fps (conform KIT-ML)
...
\subsection{Studiu ablativ: PhysicsSSM}

Comparația cu și fără poarta PhysicsSSM (pe 300 de secvențe de validare KIT-ML):

\begin{table}[h]
\centering
\begin{tabular}{l c c}
\hline
\textbf{Metrică} & \textbf{MotionSSM} & \textbf{MotionSSM + PhysicsSSM} \\
\hline
Val Loss (reconstrucție) & 0,37 & 0,097 \\
Val Loss (fizică) & --- & 0,21 \\
Foot sliding (cadre afectate) & necontrolat & penalizat \\
Ground penetration & necontrolat & penalizat \\
Jerk normalizat & necontrolat & penalizat \\
\hline
\end{tabular}
\caption{Ablație PhysicsSSM pe setul de validare KIT-ML}
\label{tab:ablation}
\end{table}

PhysicsSSM reduce pierderea de reconstrucție de la 0,37 la 0,097 (reducere de $\sim$74\%), confirmând că integrarea constrângerilor fizice îmbunătățește calitatea mișcării generate.
...
Limitele articulațiilor (Range of Motion) au fost calibrate pe secvențe KIT-ML reale:
\begin{itemize}
    \item Rata de validare inițială pe date reale: 48,9\% (limitele erau prea stricte)
    \item După calibrare: \textbf{100\%} pe datele KIT-ML de antrenare
