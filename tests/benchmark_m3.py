...
_TEST_SPHERE = "12. Generate sphere"
_TEST_SIZE   = "13. Mesh file size > 0"
_TEST_PATH   = "14. Mesh path valid"
_TEST_LABEL  = "15. Backend label"
...
class M3Benchmark:
    ...
    def _test_backend_setup(self, has_cuda: bool) -> None:
        print("\n[9-11] BACKEND SETUP")
        print("-" * 50)
    ...
    def _test_generation(self) -> None:
        ...
        if not (self.generator and self.generator._is_setup):
            for name in (_TEST_SPHERE, _TEST_SIZE, _TEST_PATH, _TEST_LABEL):
                self.test(name, False, _NOT_READY)
            return
        ...
        try:
            ...
            self.test(_TEST_SPHERE, result is not None and os.path.exists(result.mesh_path))
        except Exception as e:
            self.test(_TEST_SPHERE, False, str(e)[:40])

        if result and os.path.exists(result.mesh_path):
            size = os.path.getsize(result.mesh_path)
            self.test(_TEST_SIZE, size > 1000, f"size={size}")
        else:
            self.test(_TEST_SIZE, False)

        self.test(_TEST_PATH, bool(result) and result.mesh_path.endswith(('.obj', '.ply')))
        self.test(_TEST_LABEL, bool(result) and result.backend == "shap-e")
    ...
    def run_all(self):
        ...
        self._test_imports()
        has_cuda = self._test_dependencies()
        self._test_backend_setup(has_cuda)
